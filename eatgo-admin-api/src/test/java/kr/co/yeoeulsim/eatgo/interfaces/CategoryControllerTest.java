...
class CategoryControllerTest {
    ...
    public void addCategory() throws Exception {
        Category category = Category.builder().name("American Food").build();

        given(categoryService.addCategory("American Food")).willReturn(category);
        mvc.perform(post("/category")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("{\"name\":\"American Food\"}"))
                .andExpect(status().isCreated())
                .andExpect(content().string("{}"));

        verify(categoryService).addCategory("American Food");
    }
}
